...
### ⚡ 优化
- **设置页面重组**: 沉浸模式和 OBS 设置从扁平列表改为分组展示，更清晰易找
- **OBS 叠加层**: 关闭叠加层时不再残留连接，可立即停止
- **护眼模式**: 切换护眼颜色后立即生效，无需重启

### 🐛 修复
- **悬浮便签重启丢失**: 修复关闭 Obsidian 后重新打开时，悬浮便签位置和内容不再恢复的问题
- **设置重启丢失**: 修复沉浸模式、OBS 等设置修改后重启 Obsidian 会恢复为旧设置的问题
...
### ✨ 新增功能
- **复制本文档**: 全平台支持一键复制当前文档，并在首行自动添加标题与空行，完美适配分享需求
...
### ⚡ 优化与修复
- **字数统计稳定性 [HOT]**: 采用编辑器事件与文件系统双重校验架构，引入时间戳守卫，彻底解决重命名、启动扫描或自动保存导致的字数统计跳变/重复计算问题
