...
- 实时统计字数，适配中文创作平台计算规则
- 章节目标、今日目标，进度可视化
- **严格章节模式**: [NEW] 字数统计、进度条及提醒仅在符合规则的章节中生效，写大纲不计入进度
- **字数实时提醒**: [NEW] 在编辑器左侧行号区域实时显示累计字数提醒，支持自定义间隔
- **高稳定性架构**: [HOT] 采用编辑器事件与文件系统双重校验，彻底解决字数统计跳变问题
...
- **选项卡式设置**: [NEW] 模块化设置页面，分类更清晰，操作更便捷
- **伏笔管理**: 标注、追踪、多章节回收
- **时间线系统**: 事件记录、多章节关联、类型分类
- **智能章节排序**：自动识别章节编号排序，支持“大纲”、“番外”等具名章节
- **规则排序优化**：[NEW] 设置页面支持通过 ▲/▼ 按钮自由调整排序规则优先级
- **合并章节**：一键合并文件夹内所有章节
...
- 浮动字数统计窗口（移动端）
- 触摸优化，防误触
- **复制本文档**: [NEW] 一键复制当前章节，并在首行自动添加文档标题，完美适配社交媒体与发布平台分享
- 跨设备字数统计同步优化
